import{_ as a}from"./plugin-vue_export-helper-DlAUqK2U.js";import{c as s,f as t,o as e}from"./app-BJ0txEXQ.js";const o="/DailyNotes/assets/image-20240731112258817-B9d_TR8H.png",p="/DailyNotes/assets/image-20240731135850186-7Xll9j1g.png",c="/DailyNotes/assets/image-20240731161711817-J-MrnIVa.png",l="/DailyNotes/assets/image-20240731164211191-9wCy7Y82.png",u={};function i(r,n){return e(),s("div",null,n[0]||(n[0]=[t(`<h1 id="_12-反射" tabindex="-1"><a class="header-anchor" href="#_12-反射"><span>12. 反射</span></a></h1><h2 id="_12-1-为何需要反射" tabindex="-1"><a class="header-anchor" href="#_12-1-为何需要反射"><span>12.1. 为何需要反射?</span></a></h2><p>在 Go 语言中，反射（reflection）是一种强大的机制，允许程序在运行时检查类型信息、操作变量、调用方法等。Go 的反射包 <code>reflect</code> 提供了一组功能，让程序能够动态地检查和操作变量、结构体、接口等信息。</p><p>以下是一些常见情况下使用反射的例子和原因：</p><ol><li><strong>通用编程</strong>：反射允许编写通用代码，能够在运行时处理<strong>不同类型的数据结构，而不需要提前知道其类型。</strong></li><li><strong>序列化和反序列化</strong>：通过反射，可以动态地将结构体转换为 JSON、XML 等格式，或者将这些格式转换回结构体。</li><li><strong>动态调用方法</strong>：反射允许程序在运行时调用结构体的方法，而不需要提前知道这些方法的名称。</li><li><strong>类型检查和类型断言</strong>：反射可以用于检查变量的类型，并进行类型断言，特别在处理接口时非常有用。</li><li><strong>代码生成</strong>：某些情况下，反射可以用于生成代码，例如在 ORM（对象关系映射）库中动态创建数据库查询语句。</li></ol><p>一个格式化的函数，我们首先用switch类型分支来测试输入参数是否实现了String方法，如果是的话就调用该方法。然后继续增加类型测试分支，检查这个值的动态类型是否是string、int、bool等基础类型，并在每种情况下执行相应的格式化操作。</p><pre><code class="language-go"><span class="token keyword">func</span> <span class="token function">Sprint</span><span class="token punctuation">(</span>x <span class="token keyword">interface</span><span class="token punctuation">{</span><span class="token punctuation">}</span><span class="token punctuation">)</span> <span class="token builtin">string</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><p>函数<code>reflect.TypeOf</code> 接受任意的 <code>interface{} </code>类型，并以<code>reflect.Type</code>形式返回其动态类型：</p><pre><code class="language-Go">t := reflect.TypeOf(3)  // a reflect.Type
fmt.Println(t.String()) // &quot;int&quot;
fmt.Println(t)          // &quot;int&quot;
</code></pre><p>其中<code>TypeOf(3) </code>调用将值 3 传给 <code>interface{}</code> 参数。回到 7.5节 的将一个具体的值转为接口类型会有一个隐式的接口转换操作，它会创建一个包含两个信息的接口值：操作数的动态类型（这里是 int）和它的动态的值（这里是 3）。</p><p>因为 <code>reflect.TypeOf</code> 返回的是一个动态类型的接口值，它总是返回具体的类型。因此，下面的代码将打印 <code>&quot;*os.File&quot; </code>而不是 <code>&quot;io.Writer&quot;</code>。稍后，我们将看到能够表达接口类型的<code> reflect.Type</code>。</p><pre><code class="language-Go">var w io.Writer = os.Stdout
fmt.Println(reflect.TypeOf(w)) // &quot;*os.File&quot;
</code></pre><p>要注意的是 <code>reflect.Type</code> 接口是满足 <code>fmt.Stringer</code> 接口的。因为打印一个接口的动态类型对于调试和日志是有帮助的，<code> fmt.Printf</code> 提供了一个缩写 %T 参数，内部使用<code>reflect.TypeOf</code>来输出：</p><pre><code class="language-Go">fmt.Printf(&quot;%T\\n&quot;, 3) // &quot;int&quot;
</code></pre><h3 id="reflect-value" tabindex="-1"><a class="header-anchor" href="#reflect-value"><span><code>reflect.Value</code></span></a></h3><p><code>reflect</code> 包中另一个重要的类型是 Value。一个 <code>reflect.Value</code> 可以装载任意类型的值。函数<code> reflect.ValueOf</code> 接受任意的 <code>interface{}</code> 类型，并返回一个装载着其动态值的 <code>reflect.Value</code>。和 <code>reflect.TypeOf</code> 类似，<code>reflect.ValueOf</code> 返回的结果也是具体的类型，但是 <code>reflect.Value</code> 也可以持有一个接口值。</p><pre><code class="language-Go">v := reflect.ValueOf(3) // a reflect.Value
fmt.Println(v)          // &quot;3&quot;
fmt.Printf(&quot;%v\\n&quot;, v)   // &quot;3&quot;
fmt.Println(v.String()) // NOTE: &quot;&lt;int Value&gt;&quot;
</code></pre><p>和 <code>reflect.Type</code> 类似，<code>reflect.Value</code> 也满足 <code>fmt.Stringer</code> 接口，但是除非<code> Value</code> 持有的是字符串，否则 <code>String</code> 方法只返回其类型。而使用 <code>fmt</code> 包的<code> %v</code> 标志参数会对<code> reflect.Values</code> 特殊处理。</p><p>对 Value 调用 Type 方法将返回具体类型所对应的 <code>reflect.Type</code>：</p><pre><code class="language-Go">t := v.Type()           // a reflect.Type
fmt.Println(t.String()) // &quot;int&quot;
</code></pre><p><code>reflect.ValueOf</code> 的逆操作是 <code>reflect.Value.Interface</code> 方法。它返回一个 <code>interface{}</code> 类型，装载着与 <code>reflect.Value</code> 相同的具体值：</p><pre><code class="language-Go">v := reflect.ValueOf(3) // a reflect.Value
x := v.Interface()      // an interface{}
i := x.(int)            // an int
fmt.Printf(&quot;%d\\n&quot;, i)   // &quot;3&quot;
</code></pre><p><code>reflect.Value</code> 和 <code>interface{}</code> 都能装载任意的值。所不同的是，一个空的接口隐藏了值内部的表示方式和所有方法，因此只有我们知道具体的动态类型才能使用类型断言来访问内部的值（就像上面那样），内部值我们没法访问。相比之下，一个 Value 则有很多方法来检查其内容，无论它的具体类型是什么。</p><h3 id="kind-方法" tabindex="-1"><a class="header-anchor" href="#kind-方法"><span>Kind 方法</span></a></h3><p>让我们再次尝试实现格式化函数 <code>format.Any</code>。我们使用 <code>reflect.Value</code> 的 <code>Kind</code> 方法来替代之前的类型 <code>switch</code>。虽然还是有无穷多的类型，但是它们的 kinds 类型却是有限的：Bool、String 和 所有数字类型的基础类型；Array 和 Struct 对应的聚合类型；Chan、Func、Ptr、Slice 和 Map 对应的引用类型；interface 类型；还有表示空值的 Invalid 类型。（空的 <code>reflect.Value</code> 的 <code>kind</code> 即为 Invalid。）</p><pre><code class="language-Go">package format
...
</code></pre><p>到目前为止，我们的函数将每个值视作一个不可分割没有内部结构的物品，因此它叫 <code>formatAtom</code>。对于聚合类型（结构体和数组）和接口，只是打印值的类型，对于引用类型（channels、functions、pointers、slices 和 maps），打印类型和十六进制的引用地址。虽然还不够理想，但是依然是一个重大的进步，并且 Kind 只关心底层表示，<code>format.Any</code> 也支持具名类型。例如：</p><pre><code class="language-Go">var x int64 = 1
var d time.Duration = 1 * time.Nanosecond
fmt.Println(format.Any(x))                  // &quot;1&quot;
fmt.Println(format.Any(d))                  // &quot;1&quot;
fmt.Println(format.Any([]int64{x}))         // &quot;[]int64 0x8202b87b0&quot;
fmt.Println(format.Any([]time.Duration{d})) // &quot;[]time.Duration 0x8202b87e0&quot;
</code></pre><figure><img src="`+o+`" alt="image-20240731112258817" tabindex="0" loading="lazy"><figcaption>image-20240731112258817</figcaption></figure><h2 id="_12-3-display-一个递归的值打印器" tabindex="-1"><a class="header-anchor" href="#_12-3-display-一个递归的值打印器"><span>12.3. Display，一个递归的值打印器</span></a></h2><p>接下来，让我们看看如何改善聚合数据类型的显示。构建一个用于调试用的Display函数：给定任意一个复杂类型 x，打印这个值对应的完整结构，同时标记每个元素的发现路径。</p><p>应该尽量避免在一个包的API中暴露涉及反射的接口。我们将定义一个未导出的display函数用于递归处理工作，导出的是Display函数，它只是display函数简单的包装以接受<code>interface{}</code>类型的参数：</p><pre><code class="language-Go">func Display(name string, x interface{}) {
    fmt.Printf(&quot;Display %s (%T):\\n&quot;, name, x)
    display(name, reflect.ValueOf(x))
}
</code></pre><p>在display函数中，我们使用了前面定义的打印基础类型——基本类型、函数和chan等——元素值的<code>formatAtom</code>函数，但是我们会使用<code>reflect.Value</code>的方法来递归显示复杂类型的每一个成员。在递归下降过程中，path字符串，从最开始传入的起始值（这里是“e”），将逐步增长来表示是如何达到当前值（例如“<code>e.args[0].value</code>”）的。</p><pre><code class="language-go"><span class="token keyword">package</span> format
...
</code></pre><p>让我们声明一个该类型的变量，然后看看Display函数如何显示它：</p><pre><code class="language-Go">strangelove := Movie{
...
</code></pre><figure><img src="`+p+`" alt="image-20240731135850186" tabindex="0" loading="lazy"><figcaption>image-20240731135850186</figcaption></figure><p>观察下面两个例子的区别：</p><pre><code class="language-Go">var i interface{} = 3
...
// (*&amp;i).value = 3
		...
		<span class="token keyword">for</span> <span class="token boolean">_</span><span class="token punctuation">,</span> key <span class="token operator">:=</span> <span class="token keyword">range</span> v<span class="token punctuation">.</span><span class="token function">MapKeys</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
			...
			<span class="token punctuation">}</span>
		<span class="token punctuation">}</span>
</code></pre><p><strong>练习 12.2：</strong> 增强display函数的稳健性，通过记录边界的步数来确保在超出一定限制后放弃递归。（在13.3节，我们会看到另一种探测数据结构是否存在环的技术。）</p><h2 id="_12-4-示例-编码为s表达式" tabindex="-1"><a class="header-anchor" href="#_12-4-示例-编码为s表达式"><span>12.4. 示例: 编码为S表达式</span></a></h2><p>这节先不看了，似乎没啥意思</p><h2 id="_12-5-通过reflect-value修改值" tabindex="-1"><a class="header-anchor" href="#_12-5-通过reflect-value修改值"><span>12.5. 通过reflect.Value修改值</span></a></h2><p>回想一下，Go语言中类似<code>x</code>、<code>x.f[1]</code>和<code>*p</code>形式的表达式都可以表示变量，但是其它如<code>x + 1</code>和<code>f(2)</code>则不是变量。一个变量就是一个可寻址的内存空间，里面存储了一个值，并且存储的值可以通过内存地址来更新。</p><p>对于<code>reflect.Values</code>也有类似的区别。有一些<code>reflect.Values</code>是可取地址的；其它一些则不可以。考虑以下的声明语句：</p><pre><code class="language-Go">x := 2                   // value   type    variable?
a := reflect.ValueOf(2)  // 2       int     no
b := reflect.ValueOf(x)  // 2       int     no
c := reflect.ValueOf(&amp;x) // &amp;x      *int    no
d := c.Elem()            // 2       int     yes (x)
</code></pre><p>其中<code>a</code>对应的变量不可取地址。因为a中的值仅仅是整数2的拷贝副本。<code>b</code>中的值也同样不可取地址。<code>c</code>中的值还是不可取地址，它只是一个指针<code>&amp;x</code>的拷贝。实际上，所有通过<code>reflect.ValueOf(x)</code>返回的<code>reflect.Value</code>都是不可取地址的。但是对于<code>d</code>，它是<code>c</code>的解引用方式生成的，指向另一个变量，因此是可取地址的。我们可以通过调用<code>reflect.ValueOf(&amp;x).Elem()</code>，来获取任意变量<code>x</code>对应的可取地址的<code>Value</code>。</p><p>我们可以通过调用<code>reflect.Value</code>的<code>CanAddr</code>方法来判断其是否可以被取地址：</p><pre><code class="language-Go">fmt.Println(a.CanAddr()) // &quot;false&quot;
fmt.Println(b.CanAddr()) // &quot;false&quot;
fmt.Println(c.CanAddr()) // &quot;false&quot;
fmt.Println(d.CanAddr()) // &quot;true&quot;
</code></pre><p>每当我们通过指针间接地获取的<code>reflect.Value</code>都是可取地址的，即使开始的是一个不可取地址的<code>Value</code>。在反射机制中，所有关于是否支持取地址的规则都是类似的。以此类推，<code>reflect.ValueOf(e).Index(i)</code>对应的值也是可取地址的，即使原始的<code>reflect.ValueOf(e)</code>不支持也没有关系。</p><p>要从变量对应的可取地址的<code>reflect.Value</code>来访问变量需要三个步骤。第一步是调用<code>Addr()</code>方法，它返回一个<code>Value</code>，里面保存了指向变量的指针。然后是在<code>Value</code>上调用<code>Interface()</code>方法，也就是返回一个<code>interface{}</code>，里面包含指向变量的指针。最后，如果我们知道变量的类型，我们可以使用类型的断言机制将得到的<code>interface{}</code>类型的接口强制转为普通的类型指针。这样我们就可以通过这个普通指针来更新变量了：</p><pre><code class="language-Go">x := 2
d := reflect.ValueOf(&amp;x).Elem()   // d refers to the variable x
px := d.Addr().Interface().(*int) // px := &amp;x
*px = 3                           // x = 3
fmt.Println(x)                    // &quot;3&quot;
</code></pre><p>或者，不使用指针，而是通过调用可取地址的<code>reflect.Value</code>的<code>reflect.Value.Set</code>方法来更新对应的值：</p><pre><code class="language-Go">d.Set(reflect.ValueOf(4))
fmt.Println(x) // &quot;4&quot;
</code></pre><p><code>Set</code>方法将在运行时执行和编译时进行类似的可赋值性约束的检查。以上代码，变量和值都是int类型，但是如果变量是int64类型，那么程序将抛出一个panic异常，所以关键问题是要确保改类型的变量可以接受对应的值：</p><pre><code class="language-Go">d.Set(reflect.ValueOf(int64(5))) // panic: int64 is not assignable to int
</code></pre><p>同样，对一个不可取地址的<code>reflect.Value</code>调用Set方法也会导致panic异常：</p><pre><code class="language-Go">x := 2
b := reflect.ValueOf(x)
b.Set(reflect.ValueOf(3)) // panic: Set using unaddressable value
</code></pre><p>这里有很多用于基本数据类型的Set方法：<code>SetInt</code>、<code>SetUint</code>、<code>SetString</code>和<code>SetFloat</code>等。</p><pre><code class="language-Go">d := reflect.ValueOf(&amp;x).Elem()
d.SetInt(3)
fmt.Println(x) // &quot;3&quot;
</code></pre><p>从某种程度上说，这些Set方法总是尽可能地完成任务。以<code>SetInt</code>为例，只要变量是某种类型的有符号整数就可以工作，即使是一些命名的类型、甚至只要底层数据类型是有符号整数就可以，而且如果对于变量类型值太大的话会被自动截断。但需要谨慎的是：对于一个引用<code>interface{}</code>类型的<code>reflect.Value</code>调用<code>SetInt</code>会导致panic异常，即使那个<code>interface{}</code>变量对于整数类型也不行。</p><pre><code class="language-Go">x := 1
...
</code></pre><p>当我们用<code>Display</code>显示<code>os.Stdout</code>结构时，我们发现反射可以越过Go语言的导出规则的限制读取结构体中未导出的成员，比如在类Unix系统上<code>os.File</code>结构体中的<code>fd int</code>成员。然而，利用反射机制并不能修改这些未导出的成员：</p><pre><code class="language-Go">stdout := reflect.ValueOf(os.Stdout).Elem() // *os.Stdout, an os.File var
fmt.Println(stdout.Type())                  // &quot;os.File&quot;
fd := stdout.FieldByName(&quot;fd&quot;)
fmt.Println(fd.Int()) // &quot;1&quot;
fd.SetInt(2)          // panic: unexported field
</code></pre><p>一个可取地址的<code>reflect.Value</code>会记录一个结构体成员是否是未导出成员，如果是的话则拒绝修改操作。因此，<code>CanAddr</code>方法并不能正确反映一个变量是否是可以被修改的。</p><p>相关的方法<code>CanSet</code>是用于检查对应的<code>reflect.Value</code>是否是可取地址并可被修改的：</p><pre><code class="language-Go">fmt.Println(fd.CanAddr(), fd.CanSet()) // &quot;true false&quot;
</code></pre><blockquote><p>可取地址不一定能被修改，能被修改一定可取地址</p></blockquote><h2 id="_12-7-获取结构体字段标签" tabindex="-1"><a class="header-anchor" href="#_12-7-获取结构体字段标签"><span>12.7. 获取结构体字段标签</span></a></h2><p>对于一个web服务，大部分HTTP处理函数要做的第一件事情就是展开请求中的参数到本地变量中。我们定义了一个工具函数，叫<code>params.Unpack</code>，通过使用结构体成员标签机制来让HTTP处理函数解析请求参数更方便。</p><p>下面的search函数是一个HTTP请求处理函数。它定义了一个匿名结构体类型的变量，用结构体的每个成员表示HTTP请求的参数。其中结构体成员标签指明了对于请求参数的名字，为了减少URL的长度这些参数名通常都是神秘的缩略词。Unpack将请求参数填充到合适的结构体成员中，这样我们可以方便地通过合适的类型类来访问这些参数。</p><pre><code class="language-Go">// search implements the /search URL endpoint.
...
</code></pre><p>最后，Unpack遍历HTTP请求的name/valu参数键值对，更新相应的结构体成员。。如果同一个名字的参数可能出现多次，并且对应的结构体成员是一个slice，那么就将所有的参数添加到slice中。对应的成员值将被覆盖，只有最后一次出现的参数值才是起作用的。</p><p>populate函数用请求的字符串类型参数值来填充单一的成员v（或者是slice类型成员中的单一的元素）。</p><pre><code class="language-go"><span class="token keyword">func</span> <span class="token function">populate</span><span class="token punctuation">(</span>v reflect<span class="token punctuation">.</span>Value<span class="token punctuation">,</span> value <span class="token builtin">string</span><span class="token punctuation">)</span> <span class="token builtin">error</span> <span class="token punctuation">{</span>
    <span class="token keyword">switch</span> v<span class="token punctuation">.</span><span class="token function">Kind</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
</code></pre><figure><img src="`+c+`" alt="image-20240731161711817" tabindex="0" loading="lazy"><figcaption>image-20240731161711817</figcaption></figure><h2 id="_12-8-显示一个类型的方法集" tabindex="-1"><a class="header-anchor" href="#_12-8-显示一个类型的方法集"><span>12.8. 显示一个类型的方法集</span></a></h2><pre><code class="language-Go">// Print prints the method set of the value x.
