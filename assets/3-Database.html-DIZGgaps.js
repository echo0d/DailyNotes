import{_ as a}from"./plugin-vue_export-helper-DlAUqK2U.js";import{c as s,a as t,o as p}from"./app-CQsA8Yx3.js";const e="/DailyNotes/assets/image-20231231132427138-CS1jBQlA.png",o="/DailyNotes/assets/image-20240108213703085-p5-lLUe2.png",c={};function l(u,n){return p(),s("div",null,n[0]||(n[0]=[t(`<h1 id="_3-java代码审计基础-数据库" tabindex="-1"><a class="header-anchor" href="#_3-java代码审计基础-数据库"><span>3-Java代码审计基础-数据库</span></a></h1><p>执行sql语句的几种方式：JDBC、Hibernate、Mybatis，三者区别如下：</p><ul><li>jdbc是较底层的持久化操作方式，而hibernate和mybatis都是在jdbc的基础上进行了封装使其更加方便程序运对持久层的操作。</li><li>jdbc就是先创建connection连接数据库，然后创建statement对象，通过statement对象执行sql语句，得到resultSet对象，通过对resultSet对象的遍历操作来获取数据并手动转为javaBean，最后关闭resultSet、statement、connection释放资源；hibernate是将数据库中的数据表映射为持久层的java对象，对sql语句修改和优化困难；mybatis将sql语句的输入参数和输出参数映射为java对象，sql语句修改和优化方便；</li><li>若进行底层编程，且对性能要求极高的话，应采用jdbc方式；若对数据库进行完整性控制的话建议使用hibernate；若灵活使用sql语句的话建议使</li></ul><h2 id="_1、jdbc" tabindex="-1"><a class="header-anchor" href="#_1、jdbc"><span>1、JDBC</span></a></h2><p>JDBC(Java Database Connectivity)，是Java连接数据库操作的原生接口。JDBC是所有框架操作数据库所必须的，是数据库的统一接口标准。</p><h3 id="_1-1-一般步骤" tabindex="-1"><a class="header-anchor" href="#_1-1-一般步骤"><span>1.1 一般步骤</span></a></h3><ol><li>使用JDBC编程需要链接数据库，注册驱动和数据库信息。</li><li>操作Connection，打开Statement对象。</li><li>通过Statement执行SQL语句，返回结果放到ResultSet对象。</li><li>使用ResultSet读取数据。</li><li>关闭数据库相关的资源。</li></ol><p>数据库：</p><pre><code>CREATE DATABASE IF NOT EXISTS CodeAudit;
use CodeAudit;
CREATE TABLE users (
  ID INT,
  name VARCHAR(50),
...
</code></pre><p>先添加pom依赖</p><pre><code>    &lt;dependencies&gt;
        &lt;dependency&gt;
...
</code></pre><p>然后是配置文件application.properties</p><pre><code># 数据源配置
...
# 其他应用配置...
</code></pre><blockquote><p>此处配置文件如果没写这么多，可以在后面代码注释掉的地方进行数据源的配置</p></blockquote><p>spring的启动函数</p><pre><code>package com.echo0d;
...
</code></pre><h2 id="_2、mybatis" tabindex="-1"><a class="header-anchor" href="#_2、mybatis"><span>2、Mybatis</span></a></h2><p>JDBC缺点：</p><ul><li>数据库连接的频繁创建、释放浪费资源进而影响系统性能。</li><li>sql代码写在 Java文件当中，如果在开发过程中我们改动某个sql，就需要去修改Java代码，改完之后还需要重新编译。</li><li>对结果集的解析也是硬编码，sql变化会导致解析结果的代码也跟着变化，系统不易维护</li></ul><h3 id="_2-1-一般步骤" tabindex="-1"><a class="header-anchor" href="#_2-1-一般步骤"><span>2.1 一般步骤</span></a></h3><p>先建表</p><pre><code>CREATE TABLE \`user\` (
...
11:34:34&#39;,&#39;女&#39;,&#39;深圳&#39;),(4,&#39;光头强&#39;,&#39;2018-03-04 12:04:06&#39;,&#39;男&#39;,&#39;广州&#39;);
</code></pre><p>整体项目结构如下</p><figure><img src="`+o+`" alt="image-20240108213703085" tabindex="0" loading="lazy"><figcaption>image-20240108213703085</figcaption></figure><p>添加maven依赖</p><pre><code class="language-xml">    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>dependencies</span><span class="token punctuation">&gt;</span></span>
        ...
        <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">&gt;</span></span>
    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependencies</span><span class="token punctuation">&gt;</span></span>
</code></pre><p>编写User的实体类</p><pre><code>package com.echo0d.entity;
...
</code></pre><p>编写UserDao的接口和方法</p><pre><code>package com.echo0d.dao;
...
</code></pre><p>在resources目录下，创建mapper文件夹。编写UserDao.xml的配置文件，导入约束文件。</p><pre><code>&lt;?xml version=&quot;1.0&quot; encoding=&quot;UTF-8&quot;?&gt;
&lt;!DOCTYPE mapper
...
</code></pre><p>在resources文件夹下创建mybatis的配置文件，这个文件后面需要在使用mybatis时候引入，例如下面的测试类中</p><pre><code>&lt;configuration&gt;
...
</code></pre><p>测试类MyBatisTest.java</p><pre><code>/**
 * @author : echo0d
 * @date : 2024/1/8 21:14
 * @Description :
 */
...
</code></pre><p>根据全局配置文件得到 sqlSessionFactory；</p><p>使用sqlSession工程，获取到 sqlSession 对象使用他来执行增删改查，一个 sqlSession 就是代表和数据库的一次会话，用完关闭；</p><p>使用 SQL 的唯一标识来告诉 MyBatis 执行那个 SQL，SQL都在保存在 SQL 映射文件中；</p>`,56)]))}const k=a(c,[["render",l]]),g=JSON.parse('{"path":"/CodeAudittutorial/1-JavaBase/3-Database.html","title":"3-Java代码审计基础-数据库","lang":"zh-CN","frontmatter":{"category":"代码审计","tag":"Java","description":"3-Java代码审计基础-数据库 执行sql语句的几种方式：JDBC、Hibernate、Mybatis，三者区别如下： jdbc是较底层的持久化操作方式，而hibernate和mybatis都是在jdbc的基础上进行了封装使其更加方便程序运对持久层的操作。 jdbc就是先创建connection连接数据库，然后创建statement对象，通过stat...","head":[["meta",{"property":"og:url","content":"https://echo0d.github.io/DailyNotes/CodeAudittutorial/1-JavaBase/3-Database.html"}],["meta",{"property":"og:site_name","content":"echo0d-notes"}],["meta",{"property":"og:title","content":"3-Java代码审计基础-数据库"}],["meta",{"property":"og:description","content":"3-Java代码审计基础-数据库 执行sql语句的几种方式：JDBC、Hibernate、Mybatis，三者区别如下： jdbc是较底层的持久化操作方式，而hibernate和mybatis都是在jdbc的基础上进行了封装使其更加方便程序运对持久层的操作。 jdbc就是先创建connection连接数据库，然后创建statement对象，通过stat..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2025-04-11T02:11:10.000Z"}],["meta",{"property":"article:author","content":"echo0d"}],["meta",{"property":"article:tag","content":"Java"}],["meta",{"property":"article:modified_time","content":"2025-04-11T02:11:10.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"3-Java代码审计基础-数据库\\",\\"image\\":[\\"\\"],\\"dateModified\\":\\"2025-04-11T02:11:10.000Z\\",\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"echo0d\\",\\"url\\":\\"\\"}]}"]]},"headers":[{"level":2,"title":"1、JDBC","slug":"_1、jdbc","link":"#_1、jdbc","children":[{"level":3,"title":"1.1 一般步骤","slug":"_1-1-一般步骤","link":"#_1-1-一般步骤","children":[]},{"level":3,"title":"1.2 数据源","slug":"_1-2-数据源","link":"#_1-2-数据源","children":[]}]},{"level":2,"title":"2、Mybatis","slug":"_2、mybatis","link":"#_2、mybatis","children":[{"level":3,"title":"2.1 一般步骤","slug":"_2-1-一般步骤","link":"#_2-1-一般步骤","children":[]}]}],"git":{"createdTime":1704282270000,"updatedTime":1744337470000,"contributors":[{"name":"echo0d","email":"echo0d@163.com","commits":6}]},"readingTime":{"minutes":10.81,"words":3242},"filePathRelative":"CodeAudittutorial/1-JavaBase/3-Database.md","localizedDate":"2024年1月3日","excerpt":"\\n<p>执行sql语句的几种方式：JDBC、Hibernate、Mybatis，三者区别如下：</p>\\n<ul>\\n<li>jdbc是较底层的持久化操作方式，而hibernate和mybatis都是在jdbc的基础上进行了封装使其更加方便程序运对持久层的操作。</li>\\n<li>jdbc就是先创建connection连接数据库，然后创建statement对象，通过statement对象执行sql语句，得到resultSet对象，通过对resultSet对象的遍历操作来获取数据并手动转为javaBean，最后关闭resultSet、statement、connection释放资源；hibernate是将数据库中的数据表映射为持久层的java对象，对sql语句修改和优化困难；mybatis将sql语句的输入参数和输出参数映射为java对象，sql语句修改和优化方便；</li>\\n<li>若进行底层编程，且对性能要求极高的话，应采用jdbc方式；若对数据库进行完整性控制的话建议使用hibernate；若灵活使用sql语句的话建议使</li>\\n</ul>","autoDesc":true}');export{k as comp,g as data};
