...
</code></pre><h3 id="_2-3-es6-module" tabindex="-1"><a class="header-anchor" href="#_2-3-es6-module"><span>2.3. ES6 module</span></a></h3><p>AMD 、 CMD 等都是在原有JS语法的基础上二次封装的一些方法来解决模块化的方案，ES6 module（在很多地方被简写为 ESM）是语言层面的规范，ES6 module 旨在为浏览器和服务器提供通用的模块解决方案。长远来看，未来无论是基于 JS 的 WEB 端，还是基于 node 的服务器端或桌面应用，模块规范都会统一使用 ES6 module。</p><h4 id="兼容性" tabindex="-1"><a class="header-anchor" href="#兼容性"><span>兼容性</span></a></h4>`,83)),s("p",null,[n[1]||(n[1]=a("目前，无论是浏览器端还是 node ，都没有完全原生支持 ES6 module，如果使用 ES6 module ，可借助 ")),s("a",k,[n[0]||(n[0]=a("babel")),o(p)]),n[2]||(n[2]=a(" 等编译器。本文只讨论 ES6 module 语法，故不对 babel 或 typescript 等可编译 ES6 的方式展开讨论。"))]),n[10]||(n[10]=t(`<h4 id="导出接口" tabindex="-1"><a class="header-anchor" href="#导出接口"><span>导出接口</span></a></h4><p>CommonJS 中顶层作用域不是全局作用域，同样的，ES6 module 中，一个文件就是一个模块，文件的顶层作用域也不是全局作用域。导出接口使用<code>export</code>关键字，导入接口使用<code>import</code>关键字。</p><p><code>export</code>导出接口有以下方式：</p><p><strong>方式1</strong></p><pre><code class="language-javascript"><span class="token keyword">export</span> <span class="token keyword">const</span> prefix <span class="token operator">=</span> <span class="token string">&#39;https://github.com&#39;</span><span class="token punctuation">;</span>
...
</code></pre>`,36)),s("p",null,[n[4]||(n[4]=a("特别说明： 该功能的提议目前处于 TC39 流程的第4阶段。更多说明，请查看")),s("a",r,[n[3]||(n[3]=a("TC39/proposal-dynamic-import")),o(p)]),n[5]||(n[5]=a("。"))]),n[11]||(n[11]=t(`<h4 id="commonjs-和-es6-module" tabindex="-1"><a class="header-anchor" href="#commonjs-和-es6-module"><span>CommonJS 和 ES6 module</span></a></h4><p>CommonJS 和 AMD 是运行时加载，在运行时确定模块的依赖关系。 ES6 module 是在编译时（<code>import()</code>是运行时加载）处理模块依赖关系，。</p><p><strong>CommonJS</strong></p><p>CommonJS 在导入模块时，会加载该模块，所谓“CommonJS 是运行时加载”，正因代码在运行完成后生成<code>module.exports</code>的缘故。当然，CommonJS 对模块做了缓存处理，某个模块即使被多次多处导入，也只加载一次。</p><pre><code>// o.js
let num = 0;
function getNum() {
  return num;
}
