...
</code></pre><h3 id="ubuntu下wireshark添加root权限" tabindex="-1"><a class="header-anchor" href="#ubuntu下wireshark添加root权限"><span>ubuntu下wireshark添加root权限</span></a></h3><p>wireshark要监控eth0，但是必须要root权限才行。但是，直接用root运行程序是相当危险，也是非常不方便的。解决方法如下：</p><ol><li>添加wireshark用户组 <code>sudo groupadd wireshark </code></li><li>将dumpcap更改为wireshark用户组 <code>sudo chgrp wireshark /usr/bin/dumpcap </code></li><li>让wireshark用户组有root权限使用dumpcap <code>sudo chmod 4755 /usr/bin/dumpcap </code></li><li>将需要使用的用户名加入wireshark用户组，我的用户名是craftor <code>sudo gpasswd -a craftor wireshark </code></li></ol>`,30)),s("p",null,[n[1]||(n[1]=e("参考：")),s("a",C,[n[0]||(n[0]=e("ubuntu下wireshark添加root权限_wireshark 加入到root组里面-CSDN博客")),o(a)])]),n[31]||(n[31]=t('<h3 id="vmware-启动报错-0xc000007b" tabindex="-1"><a class="header-anchor" href="#vmware-启动报错-0xc000007b"><span>Vmware 启动报错 ‘0xc000007b‘</span></a></h3><p>原因：</p><p>C++库改变。</p><p>解决方法：</p><p>windows打开控制面板，然后打开程序，卸载程序(图中左下角)。然后找到图中两个C++程序，分别右键，卸载，系统会弹出安装/卸载程序，然后<strong>点击修复</strong>。</p><figure><img src="'+d+'" alt="image-20240813200317802" tabindex="0" loading="lazy"><figcaption>image-20240813200317802</figcaption></figure><figure><img src="'+u+`" alt="image-20240813200437702" tabindex="0" loading="lazy"><figcaption>image-20240813200437702</figcaption></figure><h3 id="kali-zsh切换为bash" tabindex="-1"><a class="header-anchor" href="#kali-zsh切换为bash"><span>Kali zsh切换为bash</span></a></h3><p>查看当前shell</p><pre><code class="language-bash"><span class="token builtin class-name">echo</span> <span class="token environment constant">$SHELL</span>
...
</code></pre><p>但感觉他还是在 C 盘到处拉屎</p><h2 id="wsl相关" tabindex="-1"><a class="header-anchor" href="#wsl相关"><span>WSL相关</span></a></h2><h3 id="wsl制作快照和回滚" tabindex="-1"><a class="header-anchor" href="#wsl制作快照和回滚"><span>WSL制作快照和回滚</span></a></h3><pre><code class="language-shell"><span class="token comment"># 查看已安装的系统</span>
...
</code></pre><p>重新启动wsl即可。</p><h3 id="wsl2使用主机的clash代理" tabindex="-1"><a class="header-anchor" href="#wsl2使用主机的clash代理"><span>WSL2使用主机的clash代理</span></a></h3>`,54)),s("p",null,[s("a",N,[n[2]||(n[2]=e("https://eastmonster.github.io/2022/10/05/clash-config-in-wsl/")),o(a)])]),s("ol",null,[s("li",null,[n[7]||(n[7]=e("在 Clash 客户端内的操作 打开客户端的允许局域网连接 (Allow LAN) 开关，如图 1 所示。 记录下客户端内的端口 (Port)，如这里是 16514. 一般默认端口为 7890. ")),s("a",O,[n[3]||(n[3]=s("img",{src:m,alt:"image.png",loading:"lazy"},null,-1)),o(a)]),n[8]||(n[8]=e(" 这个方法需要 Windows 10/11 22H2 和 WSL 2.0.5 及以上版本 你可以使用 winver 和 wsl -v 查看你的 Windows 和 WSL 版本。 如果你使用这个方法，你需要确保 Clash 客户端中设置的系统代理类型为 HTTP 而不是 PAC，否则无法代理 HTTPS 请求: ")),s("a",G,[n[4]||(n[4]=s("img",{src:h,alt:"image.png",loading:"lazy"},null,-1)),o(a)]),n[9]||(n[9]=e(" 在 WSL ")),s("a",L,[n[5]||(n[5]=e("2.0.5 版本")),o(a)]),n[10]||(n[10]=e("后，一些特性得以稳定，这里要用到的是")),s("a",M,[n[6]||(n[6]=e("镜像模式网络")),o(a)]),n[11]||(n[11]=e("。 在 C:\\Users<UserName>.wslconfig 文件中 (如果不存在就手动创建一个) 加入以下内容:"))])]),n[32]||(n[32]=t(`<pre><code>[wsl2]
...
</code></pre><ol><li>重启你当前的终端或者新开一个终端tab，这样环境变量才会生效。如果是在 <code>VS Code</code> 的终端执行命令，需要重启 <code>VS Code</code></li><li>最后正常启动项目即可，第一次启动项目可能会有点慢，因为需要执行 <code>cgo</code> 编译打包好 <code>SQlite</code> 相关的东西</li></ol>`,6)),s("blockquote",null,[s("p",null,[n[27]||(n[27]=e("来自: ")),s("a",$,[n[26]||(n[26]=e("解决 Go 中使用 Sqlite 报错 go-sqlite3 requires cgo to work - 付小晨")),o(a)])])]),n[36]||(n[36]=t('<h3 id="goland-无法debug" tabindex="-1"><a class="header-anchor" href="#goland-无法debug"><span>Goland 无法debug</span></a></h3><p>报错日志如下： couldn’t start listener: listen tcp: address 0:0:0:0:0:0:0:1:56648: too many colons in address<img src="'+S+'" alt="image.jpg" loading="lazy">我使用的Goland版本为2021.2.3 从以上报错信息可以看出是ip解析相关的问题，经过了各种办法最终找到了解决方案：</p><ol><li><strong>第一步</strong>： <img src="'+D+'" alt="image.jpg" loading="lazy"></li><li><strong>第二步</strong>： 在Goland中：Help-&gt;Edit Custom VM options, 打开文件后如下图 <img src="'+q+`" alt="image.jpg" loading="lazy"> 在最后配置如下两行</li></ol><pre><code>-Djava.net.preferIPv4Stack=true
...
</code></pre>`,8)),s("blockquote",null,[s("p",null,[n[29]||(n[29]=e("来自: ")),s("a",U,[n[28]||(n[28]=e("Goland无法debug\\地址解析出错\\无法下一步操作-CSDN博客")),o(a)])])]),n[37]||(n[37]=t('<h3 id="vs-code-集成终端上出现意外的-ansi-转义码" tabindex="-1"><a class="header-anchor" href="#vs-code-集成终端上出现意外的-ansi-转义码"><span>VS Code 集成终端上出现意外的 ANSI 转义码</span></a></h3><p>使用了oh-my-posh，在打开nodejs的项目时，我的vscode中的Windows powershell打开时出现这行代码： <img src="'+_+`" alt="image.png" loading="lazy"></p><pre><code> \\x1b7                                                                                                                                                          \\x1b[38\\x3b2\\x3b89\\x3b201\\x3b165m \\x1b[0m\\x1b[38\\x3b2\\x3b224\\x3b222\\x3b244min \\x1b[38\\x3b2\\x3b75\\x3b149\\x3b233m\\x1b[1mpwsh\\x1b[22m\\x1b[27m\\x1b[38\\x3b2\\x3b224\\x3b222\\x3b244m \\x1b[0m\\x1b[38\\x3b2\\x3b224\\x3b222\\x3b244mat \\x1b[38\\x3b2\\x3b75\\x3b149\\x3b233m\\x1b[1m09:54:21\\x1b[22m\\x1b[0m\\x1b8
