import{_ as a}from"./plugin-vue_export-helper-DlAUqK2U.js";import{c as s,b as t,o as p}from"./app-BZ3msaS8.js";const e="/DailyNotes/assets/image-20231231132427138-CS1jBQlA.png",o="/DailyNotes/assets/image-20240108213703085-p5-lLUe2.png",c={};function l(u,n){return p(),s("div",null,[...n[0]||(n[0]=[t(`<h1 id="_3-java-代码审计基础-数据库" tabindex="-1"><a class="header-anchor" href="#_3-java-代码审计基础-数据库"><span>3-Java 代码审计基础-数据库</span></a></h1><p>执行 sql 语句的几种方式：JDBC、Hibernate、Mybatis，三者区别如下：</p><ul><li>jdbc 是较底层的持久化操作方式，而 hibernate 和 mybatis 都是在 jdbc 的基础上进行了封装使其更加方便程序运对持久层的操作。</li><li>jdbc 就是先创建 connection 连接数据库，然后创建 statement 对象，通过 statement 对象执行 sql 语句，得到 resultSet 对象，通过对 resultSet 对象的遍历操作来获取数据并手动转为 javaBean，最后关闭 resultSet、statement、connection 释放资源；hibernate 是将数据库中的数据表映射为持久层的 java 对象，对 sql 语句修改和优化困难；mybatis 将 sql 语句的输入参数和输出参数映射为 java 对象，sql 语句修改和优化方便；</li><li>若进行底层编程，且对性能要求极高的话，应采用 jdbc 方式；若对数据库进行完整性控制的话建议使用 hibernate；若灵活使用 sql 语句的话建议使</li></ul><h2 id="_1、jdbc" tabindex="-1"><a class="header-anchor" href="#_1、jdbc"><span>1、JDBC</span></a></h2><p>JDBC(Java Database Connectivity)，是 Java 连接数据库操作的原生接口。JDBC 是所有框架操作数据库所必须的，是数据库的统一接口标准。</p><h3 id="_1-1-一般步骤" tabindex="-1"><a class="header-anchor" href="#_1-1-一般步骤"><span>1.1 一般步骤</span></a></h3><ol><li>使用 JDBC 编程需要链接数据库，注册驱动和数据库信息。</li><li>操作 Connection，打开 Statement 对象。</li><li>通过 Statement 执行 SQL 语句，返回结果放到 ResultSet 对象。</li><li>使用 ResultSet 读取数据。</li><li>关闭数据库相关的资源。</li></ol><p>数据库：</p><pre><code>CREATE DATABASE IF NOT EXISTS CodeAudit;
use CodeAudit;
CREATE TABLE users (
  ID INT,
  name VARCHAR(50),
...
(4, &#39;Bob Thompson&#39;, &#39;888-999-0000&#39;);
</code></pre><p>先添加 pom 依赖</p><pre><code>    &lt;dependencies&gt;
        &lt;dependency&gt;
        ...
        &lt;/dependency&gt;
    &lt;/dependencies&gt;
</code></pre><p>然后按照上面的步骤</p><pre><code class="language-java"><span class="token doc-comment comment">/**
...
<span class="token comment">//            DriverManager.registerDriver(new com.mysql.jdbc.Driver()); //使用 \`DriverManager\` 类的 \`registerDriver\` 方法来实例化驱动程序：</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><p>上图代码里面有一个<code>Class.forName(&quot;com.mysql.jdbc.Driver&quot;)</code>实际上会触发类加载，<code>com.mysql.jdbc.Driver</code>类将会被初始化，所以<code>static静态语句块</code>中的代码也将会被执行，如下图源码：</p><figure><img src="`+e+`" alt="image-20231231132427138" tabindex="0" loading="lazy"><figcaption>image-20231231132427138</figcaption></figure><p>其实也可以换个方法触发类加载，即实例化<code>com.mysql.cj.jdbc.Driver</code>或<code>com.mysql.jdbc.Driver</code>类加载即会执行静态代码块，如上面代码中注释掉的部分。</p><blockquote><p>如果反射某个类又不想初始化类方法有两种途径：</p><ol><li>使用<code>Class.forName(&quot;xxxx&quot;, false, loader)</code>方法，将第二个参数传入 false。</li><li>ClassLoader.load(&quot;xxxx&quot;);</li></ol></blockquote><p><strong>删掉 Class.forName()反射会发现依旧正常执行不报错：</strong></p><p>这里利用了 Java 的一大特性:<code>Java SPI(Service Provider Interface)</code>，因为<code>DriverManager</code>在初始化的时候会调用<code>java.util.ServiceLoader</code>类提供的 SPI 机制，Java 会自动扫描 jar 包中的<code>META-INF/services</code>目录下的文件，并且还会自动的<code>Class.forName(文件中定义的类)</code>，这也就解释了为什么不需要<code>Class.forName</code>也能够成功连接数据库的原因了</p><h3 id="_1-2-数据源" tabindex="-1"><a class="header-anchor" href="#_1-2-数据源"><span>1.2 数据源</span></a></h3><p>虽然可以直接使用 JDBC 驱动程序来创建和管理数据库连接，但使用数据源（DataSource）提供了更好的连接管理和资源控制的机制。以下是 JDBC 使用 DataSource 的一些好处：</p><ol><li>连接池管理：数据源提供了连接池的功能，可以在应用程序启动时创建一组数据库连接并放入连接池中。这样，在需要连接数据库时，可以从连接池中获取连接，而不是每次都创建新的连接和断开连接。连接池可以有效地管理和重用连接，提高性能和响应速度。</li><li>连接参数配置：数据源允许你将连接参数配置在一个地方，而不是在每个数据库连接的地方单独设置。这样可以简化代码，并提供一种集中管理连接参数的方式，方便进行统一的配置和修改。</li><li>连接错误处理：数据源可以处理连接错误和异常情况，例如网络中断、数据库崩溃等。当连接发生错误时，数据源可以自动处理连接的关闭和重新建立，确保应用程序的稳定性和可靠性。</li><li>并发控制：数据源可以提供并发控制机制，以限制同时使用的数据库连接数。这样可以避免过多的连接导致数据库性能下降，并提供一种资源控制的方式，确保数据库连接的合理使用。</li><li>支持事务管理：数据源可以与事务管理器（如 JavaEE 中的 JTA）集成，提供对事务的支持。它可以管理连接的事务性，包括事务的开始、提交和回滚，确保数据的一致性和完整性</li></ol><blockquote><p>以上来自 chatGPT</p></blockquote><p>常见的数据源有：<code>DBCP</code>、<code>C3P0</code>、<code>Druid</code>、<code>Mybatis DataSource</code>，他们都实现于<code>javax.sql.DataSource</code>接口。</p><p>如下为 druid 数据源的一个例子: pom.xml</p><pre><code class="language-xml"><span class="token prolog">&lt;?xml version=&quot;1.0&quot; encoding=&quot;UTF-8&quot;?&gt;</span>
...
</code></pre><p>然后是配置文件 application.properties</p><pre><code># 数据源配置
...
# 其他应用配置...
</code></pre><blockquote><p>此处配置文件如果没写这么多，可以在后面代码注释掉的地方进行数据源的配置</p></blockquote><p>spring 的启动函数</p><pre><code>package com.echo0d;
...
</code></pre><h2 id="_2、mybatis" tabindex="-1"><a class="header-anchor" href="#_2、mybatis"><span>2、Mybatis</span></a></h2><p>JDBC 缺点：</p><ul><li>数据库连接的频繁创建、释放浪费资源进而影响系统性能。</li><li>sql 代码写在 Java 文件当中，如果在开发过程中我们改动某个 sql，就需要去修改 Java 代码，改完之后还需要重新编译。</li><li>对结果集的解析也是硬编码，sql 变化会导致解析结果的代码也跟着变化，系统不易维护</li></ul><h3 id="_2-1-一般步骤" tabindex="-1"><a class="header-anchor" href="#_2-1-一般步骤"><span>2.1 一般步骤</span></a></h3><p>先建表</p><pre><code>CREATE TABLE \`user\` (
...
11:34:34&#39;,&#39;女&#39;,&#39;深圳&#39;),(4,&#39;光头强&#39;,&#39;2018-03-04 12:04:06&#39;,&#39;男&#39;,&#39;广州&#39;);
</code></pre><p>整体项目结构如下</p><figure><img src="`+o+`" alt="image-20240108213703085" tabindex="0" loading="lazy"><figcaption>image-20240108213703085</figcaption></figure><p>添加 maven 依赖</p><pre><code class="language-xml">    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>dependencies</span><span class="token punctuation">&gt;</span></span>
        ...
        <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependency</span><span class="token punctuation">&gt;</span></span>
    <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>dependencies</span><span class="token punctuation">&gt;</span></span>
</code></pre><p>编写 User 的实体类</p><pre><code>package com.echo0d.entity;
...
</code></pre><p>编写 UserDao 的接口和方法</p><pre><code>package com.echo0d.dao;
...
</code></pre><p>在 resources 目录下，创建 mapper 文件夹。编写 UserDao.xml 的配置文件，导入约束文件。</p><pre><code>&lt;?xml version=&quot;1.0&quot; encoding=&quot;UTF-8&quot;?&gt;
&lt;!DOCTYPE mapper
...
</code></pre><p>在 resources 文件夹下创建 mybatis 的配置文件，这个文件后面需要在使用 mybatis 时候引入，例如下面的测试类中</p><pre><code>&lt;configuration&gt;
...
&lt;!--        此处填入mapper的配置文件--&gt;
    ...
    &lt;/mappers&gt;
&lt;/configuration&gt;
</code></pre><p><strong>注意</strong>：一定要完成两个绑定</p><pre><code>（1）Mapper 接口与 Mapper 映射文件的绑定，在 Mapper 映射文件中的 &lt;mapper&gt; 标签中的 namespace 中必须指定 Mapper 接口的全类名

（2）Mapper 映射文件中的增删改查标签的 id 必须指定 Mapper 接口中的方法名；
</code></pre><p>测试类 MyBatisTest.java</p><pre><code>/**
 * @author : echo0d
 * @date : 2024/1/8 21:14
 * @Description :
 */
...
</code></pre><p>根据全局配置文件得到 sqlSessionFactory；</p><p>使用 sqlSession 工程，获取到 sqlSession 对象使用他来执行增删改查，一个 sqlSession 就是代表和数据库的一次会话，用完关闭；</p><p>使用 SQL 的唯一标识来告诉 MyBatis 执行那个 SQL，SQL 都在保存在 SQL 映射文件中；</p>`,56)])])}const k=a(c,[["render",l]]),g=JSON.parse('{"path":"/CodeAudittutorial/1-JavaBase/3-Database.html","title":"3-Java 代码审计基础-数据库","lang":"zh-CN","frontmatter":{"category":"代码审计","tag":"Java","description":"3-Java 代码审计基础-数据库 执行 sql 语句的几种方式：JDBC、Hibernate、Mybatis，三者区别如下： jdbc 是较底层的持久化操作方式，而 hibernate 和 mybatis 都是在 jdbc 的基础上进行了封装使其更加方便程序运对持久层的操作。 jdbc 就是先创建 connection 连接数据库，然后创建 stat...","head":[["meta",{"property":"og:url","content":"https://echo0d.github.io/DailyNotes/CodeAudittutorial/1-JavaBase/3-Database.html"}],["meta",{"property":"og:site_name","content":"echo0d-notes"}],["meta",{"property":"og:title","content":"3-Java 代码审计基础-数据库"}],["meta",{"property":"og:description","content":"3-Java 代码审计基础-数据库 执行 sql 语句的几种方式：JDBC、Hibernate、Mybatis，三者区别如下： jdbc 是较底层的持久化操作方式，而 hibernate 和 mybatis 都是在 jdbc 的基础上进行了封装使其更加方便程序运对持久层的操作。 jdbc 就是先创建 connection 连接数据库，然后创建 stat..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2025-06-04T09:27:16.000Z"}],["meta",{"property":"article:author","content":"echo0d"}],["meta",{"property":"article:tag","content":"Java"}],["meta",{"property":"article:modified_time","content":"2025-06-04T09:27:16.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"3-Java 代码审计基础-数据库\\",\\"image\\":[\\"\\"],\\"dateModified\\":\\"2025-06-04T09:27:16.000Z\\",\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"echo0d\\",\\"url\\":\\"\\"}]}"]]},"headers":[{"level":2,"title":"1、JDBC","slug":"_1、jdbc","link":"#_1、jdbc","children":[{"level":3,"title":"1.1 一般步骤","slug":"_1-1-一般步骤","link":"#_1-1-一般步骤","children":[]},{"level":3,"title":"1.2 数据源","slug":"_1-2-数据源","link":"#_1-2-数据源","children":[]}]},{"level":2,"title":"2、Mybatis","slug":"_2、mybatis","link":"#_2、mybatis","children":[{"level":3,"title":"2.1 一般步骤","slug":"_2-1-一般步骤","link":"#_2-1-一般步骤","children":[]}]}],"git":{"createdTime":1704282270000,"updatedTime":1749029236000,"contributors":[{"name":"echo0d","email":"echo0d@163.com","commits":7}]},"readingTime":{"minutes":10.81,"words":3243},"filePathRelative":"CodeAudittutorial/1-JavaBase/3-Database.md","localizedDate":"2024年1月3日","excerpt":"\\n","autoDesc":true}');export{k as comp,g as data};
