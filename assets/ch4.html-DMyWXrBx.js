import{_ as o}from"./plugin-vue_export-helper-DlAUqK2U.js";import{c as e,f as s,a,e as t,b as c,o as u,r as l}from"./app-BJ0txEXQ.js";const i="/DailyNotes/assets/image-20240422173219723-D7-1Y00V.png",k="/DailyNotes/assets/image-20240422175054597-Dq3C6kxr.png",r="/DailyNotes/assets/image-20240424220922040-B4ZTIfjn.png",d="/DailyNotes/assets/image-20240425175118604-1igeGMbZ.png",m="/DailyNotes/assets/image-20240426180246893-0jrVYcm-.png",g={},f={href:"https://pkg.go.dev/flag",target:"_blank",rel:"noopener noreferrer"};function b(h,n){const p=l("ExternalLinkIcon");return u(),e("div",null,[n[2]||(n[2]=s(`<h1 id="_04-复合数据类型" tabindex="-1"><a class="header-anchor" href="#_04-复合数据类型"><span>04. 复合数据类型</span></a></h1><ul><li>主要讨论四种类型——数组、slice、map和结构体</li><li>演示如何使用结构体来解码和编码到对应JSON格式的数据，并且通过结合使用模板来生成HTML页面。</li></ul><p>数组和结构体是聚合类型，它们的值由许多元素或成员字段的值组成。</p><p>数组是由同构的元素组成——每个数组元素都是完全相同的类型——结构体则是由异构的元素组成的。数组和结构体都是有固定内存大小的数据结构。相比之下，slice和map则是动态的数据结构，它们将根据需要动态增长。</p><h2 id="_4-1-数组" tabindex="-1"><a class="header-anchor" href="#_4-1-数组"><span>4.1. 数组</span></a></h2><ul><li>数组是一个由固定长度的特定类型元素组成的序列，一个数组可以由零个或多个元素组成。</li><li>数组的每个元素可以通过索引下标来访问，索引下标的范围是从0开始到数组长度减1的位置。</li><li>内置的len函数将返回数组中元素的个数。</li></ul><p>因为数组的长度是固定的，因此在Go语言中很少直接使用数组。和数组对应的类型是Slice（切片），它是可以增长和收缩的动态序列，slice功能也更灵活，但是要理解slice工作原理的话需要先理解数组。</p><h3 id="数组定义" tabindex="-1"><a class="header-anchor" href="#数组定义"><span>数组定义</span></a></h3><p>默认情况下，数组的每个元素都被初始化为元素类型对应的零值，对于数字类型来说就是0。</p><pre><code class="language-Go">var a [3]int             // array of 3 integers
